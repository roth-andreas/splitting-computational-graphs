...
class PeptidesStructuralDataset(InMemoryDataset):
    def __init__(self, root='datasets', smiles2graph=smiles2graph,
        ...
        if osp.isdir(self.folder) and (not osp.exists(release_tag)):
            ...
            if True:#input("Will you update the dataset now? (y/N)\n").lower() == 'y':
                shutil.rmtree(self.folder)

        super().__init__(self.folder, transform, pre_transform)
        self.data, self.slices = torch.load(self.processed_paths[0])
        print(self.data.node_degrees_out[:10],self.data.node_degrees_out[-10:])        
    @property
    ...
    def download(self):
        if True:#decide_download(self.url):
    ...
    def process(self):
        ...
        for i in tqdm(range(len(smiles_list))):
            ...
            data.node_degrees_out = pyg.utils.degree(data.edge_index[0], data.num_nodes, dtype=torch.long)
            data.node_degrees_in = pyg.utils.degree(data.edge_index[1], data.num_nodes, dtype=torch.long)
            #data.edge_index, _ = add_remaining_self_loops(edge_index)
