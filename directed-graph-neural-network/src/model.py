...
from dag_utils import init_graph, split_graph, directed_norm_adj


def get_conv(conv_type, input_dim, output_dim, alpha, ordering_type):
    if conv_type == "gcn":
    ...
    elif conv_type == "da-sage":
        return DaDirSageConv(input_dim, output_dim, alpha)
    elif conv_type == "da-gcn":
        return DaDirGCNConv(input_dim, output_dim, alpha)
    else:
        raise ValueError(f"Convolution type {conv_type} not supported")


class DaDirSageConv(torch.nn.Module):
    def __init__(self, input_dim, output_dim, alpha):
        super(DaDirSageConv, self).__init__()

        self.input_dim = input_dim
        self.output_dim = output_dim

        self.conv_src_to_dst = DA_SAGE.DASAGEConv(input_dim, output_dim, share_init=True, cached=True, root_weight=False)
        self.conv_dst_to_src = DA_SAGE.DASAGEConv(input_dim, output_dim, share_init=True, cached=True, root_weight=False, degree_index=1)
    ...
    def forward(self, x, edge_index):
        ...
        if self.alpha > 0:
            ...
            x_ = self.alpha * self.conv_dst_to_src(x, edge_index_t)
        if self.alpha < 1:
            if x_ is None:
                x_ = (1 - self.alpha) * self.conv_src_to_dst(x, edge_index)
            else:
                x_ += (1 - self.alpha) * self.conv_src_to_dst(x, edge_index)
        return self.lin_self(x) + x_


class DaDirGCNConv(torch.nn.Module):
    def __init__(self, input_dim, output_dim, alpha):
        super(DaDirGCNConv, self).__init__()

        self.input_dim = input_dim
        self.output_dim = output_dim

        self.conv_src_to_dst = DA_GCN.DAGCNConv(input_dim, output_dim, alpha=1.0, share_init=True, add_self_loops=False, norm='dir',
                                cached=True)
        self.conv_dst_to_src = DA_GCN.DAGCNConv(input_dim, output_dim, alpha=1.0, share_init=True, add_self_loops=False, norm='dir',
                                cached=True, degree_index=1)
        self.alpha = alpha
...
class DirGCNConv(torch.nn.Module):
    ...
    def forward(self, x, edge_index):
        ...
        x_ = None
        if self.alpha > 0:
            x_ = self.alpha * self.lin_src_to_dst(self.adj_norm @ x)
        if self.alpha < 1:
            if x_ is None:
                x_ = (1 - self.alpha) * self.lin_dst_to_src(self.adj_t_norm @ x)
            else:
                x_ += (1 - self.alpha) * self.lin_dst_to_src(self.adj_t_norm @ x)
        return x_
...
class DirSageConv(torch.nn.Module):
    ...
    def forward(self, x, edge_index):
        x_ = None
        if self.alpha > 0:
            x_ = self.alpha * self.conv_dst_to_src(x, edge_index)
        if self.alpha < 1:
            if x_ is None:
                x_ = (1 - self.alpha) * self.conv_src_to_dst(x, edge_index)
            else:
...
class GNN(torch.nn.Module):
    def __init__(
        ...
        if num_layers == 1:
            self.convs = ModuleList([get_conv(conv_type, num_features, output_dim, self.alpha, ordering_type)])
        else:
            self.convs = ModuleList([get_conv(conv_type, num_features, hidden_dim, self.alpha, ordering_type)])
            for _ in range(num_layers - 2):
                self.convs.append(get_conv(conv_type, hidden_dim, hidden_dim, self.alpha, ordering_type))
            self.convs.append(get_conv(conv_type, hidden_dim, output_dim, self.alpha, ordering_type))
...
class LightingFullBatchModelWrapper(pl.LightningModule):
    ...
    def validation_step(self, batch, batch_idx):
        ...
        y_pred = out.max(1)[1]
        val_acc = self.evaluate(y_pred=y_pred[self.val_mask], y_true=batch.y[self.val_mask])
        val_loss = nn.functional.nll_loss(out[self.val_mask], batch.y[self.val_mask].squeeze())
